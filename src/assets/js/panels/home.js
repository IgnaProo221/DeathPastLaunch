...
class Home {
    ...
    async youtubebtn() {
        document.querySelector('.youtube').addEventListener('click', async() => {
            require('electron').shell.openExternal("https://www.youtube.com/channel/UCrvYQAl3mhlc4utmBbJmCuA")
        })}
    
    async discordbtn() {
    document.querySelector('.discord').addEventListener('click', async() => {
        require('electron').shell.openExternal("https://discord.gg/RPWtszmECK")
        })}
    ...
    async serverbtn() {
        document.querySelector('.server').addEventListener('click', async() => {
            require('electron').shell.openExternal("https://docs.google.com/forms/d/1_0nL263bdTorPJOe7wX_jeWerpF2TQrmEPHbT9hZmjI/edit")
        })}
